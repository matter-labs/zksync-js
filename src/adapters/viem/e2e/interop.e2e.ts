...
describe('interop.e2e (viem): send-native', () => {
  ...
  it('finalize executes bundle on destination and FundsReceiver balance increases', async () => {
    ...
    const receiverBalanceAfter = await l2Dst.getBalance({ address: fundsReceiver });
...
// ─────────────────────────────────────────────────────────────────────────────
describe('interop.e2e (viem): send-erc20', () => {
  ...
  it('finalize executes ERC-20 transfer and recipient receives tokens on destination', async () => {
    ...
    const tokenDstAddress = (await l2Dst.readContract({
    ...
    const balanceOnDst = (await l2Dst.readContract({
...
// ─────────────────────────────────────────────────────────────────────────────
describe('interop.e2e (viem): remote-call', () => {
  let sdk: any, l2Dst: any;
  ...
  beforeAll(async () => {
    ({ sdk, l2Dst } = makeInteropSetup());
    greeterAddress = await getGreetingAddress({ privateKey: PRIVATE_KEY, rpcUrl: DST_L2_RPC });
  }, 60_000);
  ...
  it('finalize executes call and Greeter message is updated on destination', async () => {
    ...
    const messageAfter = (await l2Dst.readContract({
