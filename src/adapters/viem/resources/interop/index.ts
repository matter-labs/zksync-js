...
export interface InteropResource {
  ...
  prepare(dstChain: ChainRef, input: InteropInput): Promise<InteropPlan<ViemTransactionRequest>>;
  tryPrepare(
    dstChain: ChainRef,
    input: InteropInput,
  ): Promise<
  ...
  >;
  create(dstChain: ChainRef, input: InteropInput): Promise<InteropHandle<ViemTransactionRequest>>;
  tryCreate(
    dstChain: ChainRef,
    input: InteropInput,
  ): Promise<
...
export function createInteropResource(
  ...
  async function settlementLayerChainId(): Promise<bigint> {
    return BigInt(await client.l1.getChainId());
  }
  ...
  async function isSaltUsed(ctx: BuildCtx, salt: Hex): Promise<boolean> {
    return Boolean(
      await client.l2.readContract({
        ...
        functionName: 'isInteropBundleSaltUsed',
        args: [ctx.sender, salt],
      }),
    );
  }
  ...
  async function assertDeadlineIsFuture(deadline: bigint): Promise<void> {
    ...
    const block = await client.l1.getBlock({ blockTag: 'latest' });
  ...
  async function assertAllowances(
    ...
    for (const approval of material.approvals) {
      ...
      const allowance = await client.l2.readContract({
        address: approval.token,
        abi: IERC20ABI,
        functionName: 'allowance',
        args: [material.ctx.sender, approval.spender],
      });
  ...
  async function previewFrom(material: Material, salt: Hex): Promise<AtomicInteropLegDraft> {
    ...
    const data = encodeFunctionData({
      abi: IInteropCenterABI,
      functionName: 'previewBundleHash',
      args: [
        payload.destinationChain,
        payload.starters as unknown as never[],
        payload.bundleAttributes,
      ],
    });
    const result = await client.l2.call({
      account: client.account,
      to: material.ctx.interopCenter,
      data,
    });
    if (!result.data) throw new Error('InteropCenter.previewBundleHash returned no data.');
    const bundleHash = decodeFunctionResult({
      abi: IInteropCenterABI,
      functionName: 'previewBundleHash',
      data: result.data,
    });
  ...
  async function resolveIndex(intent: AtomicInteropIntent): Promise<bigint> {
    ...
    return resolveAtomicInteropIndex({
      ...
      reader: {
        leafCount: async () =>
          await client.l2.readContract({
            address: L2_INTEROP_COMMITMENT_TREE_ADDRESS,
            abi: IL2InteropCommitmentTreeABI,
            functionName: 'leafCount',
          }),
        leafAt: async (index) => {
          const leaf = await client.l2.readContract({
            address: L2_INTEROP_COMMITMENT_TREE_ADDRESS,
            abi: IL2InteropCommitmentTreeABI,
            functionName: 'leafAt',
            args: [index],
  ...
  async function simulateSend(
    ctx: BuildCtx,
    step: PlanStep<ViemTransactionRequest>,
  ): Promise<void> {
    await client.l2.call({
      account: client.account,
  ...
  function parseBundleReceipt(
    ...
    const event = IInteropCenterABI.find(
      (item): item is Extract<(typeof IInteropCenterABI)[number], { type: 'event' }> =>
        item.type === 'event' && item.name === 'InteropBundleSent',
    );
    ...
    for (const log of receipt.logs) {
      ...
      const args = decoded.args as {
        ...
        interopBundle: {
          ...
          destinationBaseTokenAssetId: Hex;
          interopBundleSalt: Hex;
          calls: readonly {
            version: Hex;
      ...
      const encodedBundle = encodeAbiParameters([event.inputs[2]], [args.interopBundle]);
      const computedHash = keccak256(
        encodeAbiParameters(
          [
            { name: 'sourceChainId', type: 'uint256' },
            { name: 'encodedBundle', type: 'bytes' },
          ],
          [args.interopBundle.sourceChainId, encodedBundle],
        ),
      );

      if (args.l2l1MsgHash.toLowerCase() !== ZERO_HASH) {
        throw new Error('Atomic interop unexpectedly emitted a public L2-to-L1 message hash.');
      }
      if (
        args.interopBundleHash.toLowerCase() !== intent.draft.bundleHash.toLowerCase() ||
        computedHash.toLowerCase() !== args.interopBundleHash.toLowerCase()
      ) {
        throw new Error('Emitted atomic interop bundle does not match its previewed bundleHash.');
      }
      if (
        args.interopBundle.sourceChainId !== intent.draft.sourceChainId ||
        args.interopBundle.destinationChainId !== intent.draft.destinationChainId ||
        args.interopBundle.bundleAttributes.salt.toLowerCase() !== intent.draft.salt.toLowerCase()
      ) {
  ...
  const getSettlementDeadline = (params: AtomicInteropDeadlineParams): Promise<bigint> =>
    wrap(OP_INTEROP.getSettlementDeadline, async () => {
      ...
      const block = await client.l1.getBlock({ blockTag: 'latest' });
      const deadline = block.timestamp + afterSeconds;
      assertDeadline(deadline);
      return deadline;
    });
  ...
  const status = (
    ...
    wrap(OP_INTEROP.status, async () => {
      ...
      const dstProvider = resolveChainRef(dstChain);
      const [srcChainId, dstChainId, slChainId] = await Promise.all([
        client.l2.getChainId(),
        dstProvider.getChainId(),
        client.l1.getChainId(),
      ]);
      assertAtomicInteropIntent(intent, {
        sender: client.account.address,
        sourceChainId: BigInt(srcChainId),
        destinationChainId: BigInt(dstChainId),
        settlementLayerChainId: BigInt(slChainId),
      });
      const [rawLegState, rawBundleState] = await Promise.all([
        client.l2.readContract({
          address: L2_ATOMIC_FLOW_MANAGER_ADDRESS,
          abi: IAtomicFlowManagerABI,
          functionName: 'legState',
          args: [intent.flow.flowId, intent.draft.bundleHash],
        }),
        dstProvider.readContract({
          address: L2_INTEROP_HANDLER_ADDRESS,
          abi: IInteropHandlerABI,
          functionName: 'bundleStatus',
          args: [intent.draft.bundleHash],
        }),
      ]);
