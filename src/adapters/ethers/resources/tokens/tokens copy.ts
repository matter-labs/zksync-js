...
export function createTokensResource(client: EthersClient): TokensResource {
  ...
  async function assetIdOfL2(l2Token: Address): Promise<Hex> {
    return wrapAs('CONTRACT', 'tokens.assetIdOfL2', async () => {
      ...
      if (!hexEq(assetId, ethers.ZeroHash as Hex)) {
        return assetId;
      }

      const { chainId } = await client.l2.getNetwork();
      const { l2NativeTokenVault: ntvAddress } = await client.ensureAddresses();
      return ntvCodec.encodeAssetId(BigInt(chainId), ntvAddress, l2Token);
    });
  }
