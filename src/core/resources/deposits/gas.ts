...
export type AbiEncoder = (abi: any, functionName: string, args: any[]) => string;
...
export type QuoteL2GasInput = {
    ...
    tx?: CoreTransactionRequest;
    gasPerPubdata?: bigint;
    l2GasLimit?: bigint; // from user params
    overrideGasLimit?: bigint; // recursive call param or explicit override
};
...
export async function quoteL2Gas(input: QuoteL2GasInput): Promise<GasQuote | undefined> {
    ...
    // If fetchFees returns 0s, maybe we should return undefined as per original adapter logic?
    // Original: if (maxFeePerGas == null) return undefined;
    // fetchFees returns 0n on error. So let's check if it's 0n and maybe fail if strict?
    // Actually original logic returns undefined if it can't find ANY fee data.
    // I will assume 0n is valid for some chains? No, usually not.
    ...
    try {
        ...
        const pp = gasPerPubdata ?? 800n; // Default? Or should be required?
