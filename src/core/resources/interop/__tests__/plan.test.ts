...
describe('interop/plan', () => {
  ...
  describe('buildIndirectBundle', () => {
    it('builds bundle with ERC-20 approvals', () => {
      ...
      const result = buildIndirectBundle(
        params,
        baseCtx(),
        emptyAttrs,
        starterData,
    ...
    it('throws when sendErc20 action is missing asset router payload', () => {
      ...
      expect(() =>
        buildIndirectBundle(params, baseCtx(), emptyAttrs, starterData, defaultFeeInfo),
      ).toThrow('buildIndirectBundle: missing assetRouterPayload for sendErc20 action.');
    });
